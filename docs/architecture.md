...
and [`../TODO.md`](../TODO.md) for exactly what's still open.
...
- **Side effects are isolated behind small traits/wrappers** (`src/sysd/dbus.rs`
  ...
  parsing — is unit-testable fast and in isolation, without needing a live
  systemd/D-Bus session or Tier B's full container for every change.
...
wsmr and `uwsm` intentionally **share the same unit namespace** — the
static graph is kept byte-identical to upstream's own shipped units
(enforced by `tests/uwsm_unit_compat.rs`, which diffs wsmr's rendered
output against a real `uwsm` 0.26.7 install), so either tool can manage the
exact same `wayland-wm@<id>.service` a session is actually running under.
This is deliberate, not an oversight: a machine can have both installed,
switch between a `Hyprland (wsmr-managed)` and a `Hyprland (uwsm-managed)`
display-manager entry, and neither tool corrupts the other's state, subject
to the ownership rules below.

- **Ownership is proven, not assumed.** `src/units/manifest.rs` fingerprints
  every file wsmr writes, so a later run can tell "wsmr's own prior
  generation" apart from "a foreign file that happens to share a name"
  (a hand-written drop-in, or one `uwsm` wrote). wsmr never overwrites or
  deletes a path it can't prove it owns.
- **The one deliberate carve-out**: per-compositor `50_custom.conf`/tweak
  drop-ins — never the static graph itself — are reclaimed automatically
  once `start` has independently confirmed via systemd (not just this
  ownership check) that no session is currently active. Those specific
  paths are only ever machine-written by a session manager, so an
  unrecognized file there under those conditions is stale leftover state
  from a previous `uwsm`/wsmr session, not something a person hand-edited.
  `session::log_notice_to_journal` reports exactly what got reclaimed, so
  this is never silent — just not a hard stop. This is what makes switching
  between a `uwsm`-managed and a wsmr-managed login work at all; before this
  existed, the second tool would simply refuse to start.
- **A genuine content mismatch on the static graph itself still hard-stops**,
  regardless of the above — those files are supposed to be byte-identical
  between the two tools, so a real difference is worth stopping for, not
  papering over.
- **Cleanup (`stop -r`) only ever removes what wsmr's own manifest verifies
  it wrote** — a drifted or foreign entry is left in place and reported, not
  force-deleted, and the static graph is excluded from removal entirely even
  if somehow present in the manifest (see `units::plan::plan_remove_all`'s
  own doc comment for why: content alone can never distinguish "wsmr's
  copy" from "uwsm's copy" of a byte-identical file).
...
### Compatibility with upstream `uwsm`

Every flag and subcommand above was checked against upstream's own
installed source (`/usr/share/uwsm/modules/uwsm/main.py`'s argparse
definitions), not memory or documentation, and cross-verified against a
live, real `uwsm`-managed session where the behavior in question is
observable (`check is-active`, unit naming). No CLI golden-snapshot suite
exists yet (see `TODO.md`) — a future change could silently drift from
upstream again — but every divergence below was deliberately verified, not
guessed:

- **`-a`/`-e` semantics** (`start`): `-a` appends `-D` desktop names to
  other sources (upstream's default already), `-e` uses `-D` names
  exclusively, discarding the rest. `-a` is a no-op beyond parsing — its
  only job is existing as `-e`'s explicit, mutually-exclusive opposite for
  scripts/muscle memory that expect it.
- **`-F`/`--hardcode`** (`start`): bakes the resolved compositor's absolute
  path into the generated unit. A compositor that can't be resolved is a
  hard error here — upstream raises an unhandled Python exception in the
  same case, so this is stricter, not looser.
- **`-p`/`--property`** (`app`): repeatable `Property=value` unit-property
  assignments, matching upstream's `action="append"` shape exactly.
  `--unit-property` is kept as a compatible long alias.
- **`-S`/`--silent`** (`app`): a plain required-value flag (`out`/`err`/
  `both`), matching upstream — not the bare-flag-defaults-to-both shape an
  earlier wsmr revision used.
- **`-U`/`--unit-rung`**: canonical values are `run`/`home` (matching
  upstream exactly, including `$UWSM_UNIT_RUNG`'s invalid-value-warns-and-
  falls-back behavior); `runtime` is kept as a value alias for wsmr's own
  prior spelling.
- **`stop -r`'s removal marks**: comma-separated compositor ids, or
  `"tweaks"` for the fixed tweak drop-ins — matches upstream's mark
  filtering. Upstream's `generic` mark has nothing to match in wsmr, since
  wsmr never marks the static graph itself as removable (see "Ownership and
  coexistence with `uwsm`" below).
- **`.desktop` suffix preserved in compositor ids** where upstream preserves
  it — a real bug was found and fixed here: an earlier revision stripped
  `.desktop` via `trim_end_matches`, which upstream never does (the id is
  the raw main-argument basename, set before entry resolution runs).
  Confirmed against a live session's actual unit name
  (`wayland-wm@hyprland.desktop.service`).
- **Known, deliberately deferred divergences**: the static-unit deployment
  model (wsmr generates the whole graph at runtime; upstream ships most of
  it statically at build time and only generates per-compositor drop-ins —
  see "The unit graph" above), other-rung cleanup not run on every `start`,
  `wayland-wm@.service`'s `TimeoutStartSec` not syncing with
  `$UWSM_WAIT_VARNAMES_TIMEOUT`, the `-v` short flag, and `aux exec`/
  `aux readiness` accepting a few flags upstream's own parser doesn't define
  for them.
...
systemd (`just integration`, and in CI's `tier-b` job) plus real Hyprland
sessions on real hardware — a disposable test account, and, since
2026-08-30, the primary account's own real desktop via an actual
`greetd`+`noctalia-greeter` display-manager login, app-launcher chain
included. The unit graph and env-delta logic are compositor-agnostic by
design — the same as upstream uwsm's — but wsmr itself has never been run
against sway, niri, river, labwc, or anything else, and that's intentional
(see `TODO.md`). See
